...
export function DashboardShell({ business, active, children }: { business: DashboardBusiness; active: 'overview' | 'appointments'; children: ReactNode }) {
  ...
  return <main className="mx-auto w-full max-w-7xl flex-1 px-4 py-7 sm:px-6 sm:py-10 lg:py-12">
    <section className="relative isolate overflow-hidden rounded-[2rem] bg-[radial-gradient(circle_at_12%_14%,rgba(126,99,255,.4),transparent_28%),linear-gradient(118deg,#151c47,#111933_55%,#38206f)] p-6 text-white shadow-[0_22px_48px_rgba(34,37,88,.2)] sm:p-9">
      ...
      <nav className="relative mt-7 flex gap-2 border-t border-white/15 pt-5" aria-label={copy.dashboard.title}>{nav.map(({ id, href, label, Icon }) => <Link key={id} href={href} className={`inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold transition focus-visible:outline-2 focus-visible:outline-white ${active === id ? 'bg-white text-[var(--brand)] shadow-sm' : 'bg-white/8 text-white hover:bg-white/15'}`}><Icon size={16} aria-hidden="true" />{label}</Link>)}</nav>
    </section>{children}
  </main>;
}
